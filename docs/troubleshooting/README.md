...
| 2026-05-04 | [k8s-env-substitution-order](2026-05-04-k8s-env-substitution-order.md) | deployment.yaml 에서 DATABASE_URL 이 DB_PASSWORD 보다 먼저 정의되어 K8s `$(VAR)` substitution 실패. asyncpg 가 literal `"$(DB_PASSWORD)"` 로 connect → InvalidPasswordError → uvicorn exit 3 → CrashLoopBackOff. env 순서 swap 으로 해결. **k8s-runtime-state** 검증 layer 추가 | 중간 |
| 2026-05-04 | [git-sha-not-equal-image-sha](2026-05-04-git-sha-not-equal-image-sha.md) | instruction 에서 `--set global.imageTag=$(git rev-parse origin/main)` 사용. main 의 latest sha 가 GHCR image sha 와 같다고 단정 — path-filter 로 docs-only / single-service commit 시 그 sha 의 image 가 GHCR 에 없음. `scripts/latest-image-sha.sh` 신설 + chart README 에 3 단계 우선순위. **instruction-state** 검증 layer 추가 | 중간 |
| 2026-05-04 | [helm-postgres-init-double-quote](2026-05-04-helm-postgres-init-double-quote.md) | Helm sprig 의 `quote` 가 double-quote 라 init.sql 의 VALUES 절이 column reference 로 해석됨. `\gexec` 가 0 행 받아 4 service DB 가 안 만들어짐 → asyncpg InvalidCatalogNameError. `quote` → `squote` 한 줄 수정. **embedded SQL semantic** 은 helm template/kubeconform 이 못 잡는 layer | 중간 |
| 2026-05-04 | [argocd-postgres-volumeclaimtemplates-typemeta-drift](2026-05-04-argocd-postgres-volumeclaimtemplates-typemeta-drift.md) | EPIC 5 부트스트랩 후 Application 3개 모두 OutOfSync/Healthy. (1) chart 의 `volumeClaimTemplates` 에 `apiVersion/kind` 누락 → K8s 서버가 채운 TypeMeta 가 LIVE 에만 보여 영구 diff. (2) `allowEmpty: false` 가 default 라 controller normalize 시 떨어져 root → child Application diff. chart 에 TypeMeta 명시 + default 값 제거로 해결. **양성 drift 진단 = `Healthy`+`OutOfSync` 조합** | 낮음 |
| 2026-05-05 | [kind-bootstrap-port-80-blocked-by-windows-apache](2026-05-05-kind-bootstrap-port-80-blocked-by-windows-apache.md) | Docker Desktop 제거 후 native docker on WSL2 로 전환 직후 `./scripts/bootstrap.sh` 가 `0.0.0.0:80 address already in use` 로 실패. WSL2 측 `ss/lsof/docker ps` 모두 비어있었으나 Windows 호스트의 PID 7312 = `httpd.exe` 가 LISTENING. 정체는 사용자가 잊고 있던 Bitnami WAMP stack 의 `wampstackApache` 자동 기동 서비스. Docker Desktop 시절엔 자체 publish layer 가 충돌을 우회시켜 가려져 있었음. `Stop-Service` + `Set-Service -StartupType Manual` 로 해결. **WSL2 docker 포트 충돌은 양쪽 OS 모두 진단 필요** + **PID→서비스 이름은 `Get-CimInstance Win32_Service`** | 중간 |
| 2026-05-05 | [istioctl-verify-install-removed-in-1-29](2026-05-05-istioctl-verify-install-removed-in-1-29.md) | `docs/setup/istio-install.md` § 2-3-3 가 `istioctl verify-install` 을 검증 명령으로 안내했으나 1.23 부터 deprecated → 1.29 에서 제거. 사용자가 `Error: unknown command "verify-install"` 보고. 가이드 작성 시 메이저 버전 (1.29) 호환성만 확인하고 sub-command 의 호환성은 검증 안 한 CLAUDE.md A-2 위반. 가이드에서 단계 삭제 + § 2-3-4 를 § 2-3-3 으로 번호 재조정 + 1.29 의 install 검증 전략 메모 추가. **도구 메이저 ≠ 그 sub-command 의 stable** | 낮음 |
| 2026-05-31 | [trivy-gate-libgnutls30-base-image-cve](2026-05-31-trivy-gate-libgnutls30-base-image-cve.md) | EPIC 6 머지 후 CI 의 Trivy 게이트가 `account` 이미지에서 HIGH 3 / CRITICAL 2 로 빌드 차단. 원인은 코드가 아니라 base image `python:3.14-slim-bookworm` 의 OS 패키지 `libgnutls30` (+deb12u6) 신규 CVE. 모두 `Status: fixed` (+deb12u7) 라 Debian 패치 발행 ↔ base image 재빌드 사이 시간 창 문제. 4 개 Dockerfile runtime stage 에 `apt-get upgrade` 추가로 해결. **Dependabot 은 base image 태그만 추적, 같은 태그 안 OS 패치는 `apt-get upgrade` 가 메움** + **Trivy 실패 = 보안 게이트 작동 신호, fixed/affected 로 분류 후 패치 vs 수용 판단** | 중간 |
