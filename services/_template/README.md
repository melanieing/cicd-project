...
> **`source .venv/bin/activate` 를 쓰지 않는 이유**: activate 는 PATH 만 바꾸기 때문에
> system 에 같은 이름의 도구(`pytest`, `pip`)가 있으면 활성화가 누락되거나 우선순위 문제로
> system 도구가 잡힐 수 있다. `./.venv/bin/<binary>` 로 직접 호출하면 절대로 그 venv 의 도구만 쓰임.
